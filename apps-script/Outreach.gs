...
function setupTriggers() {
  ...
  ScriptApp.newTrigger('runNightlyOutreach').timeBased().everyDays(1).atHour(7).create();
  ScriptApp.newTrigger('checkReplies').timeBased().everyDays(1).atHour(12).create();
...
function gmailSend_(to, subject, plainText, htmlText, replyToThreadId) {
  ...
  if (response.getResponseCode() !== 200) {
    throw new Error('Gmail API ' + response.getResponseCode() + ': ' + response.getContentText());
  }
  return JSON.parse(response.getContentText());
}
